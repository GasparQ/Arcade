    ...
    highScores(gameName + ".arcade")
{
    ...
    // All scores
    for (int i = 0, len = static_cast<int>(scores.size()); i < len && i < 10; ++i)
    {
...
{
    for (size_t i = 0; i < HighScoreComponent::componentNb; ++i)
    {
        delete(components[i]);
    }
}
...
void HighScoreComponent::UpdatePseudo(const int key)
{
    ...
    if (name.empty())
        name = HighScoreComponent::pseudoPlaceholder;
    ...
    components[HighScoreComponent::ComponentsPos::PSEUDO] = new UIComponent(
            Vector2<int>(static_cast<int>(ArcadeSystem::winWidth) / 2 - static_cast<int>(name.length()) / 2, 2),
            AComponent::ComponentColor::COLOR_WHITE,
            Vector2<int>(static_cast<int>(name.length()), 1),
            name
    );
}
...
void HighScoreComponent::submit() const
{
    highScores.flush();
}
